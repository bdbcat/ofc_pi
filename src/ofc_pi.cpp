...
int ofc_pi::GetAPIVersionMajor()
{
      return MY_API_VERSION_MAJOR;
}

int ofc_pi::GetAPIVersionMinor()
{
      return MY_API_VERSION_MINOR;
}

int ofc_pi::GetPlugInVersionMajor()
{
      return PLUGIN_VERSION_MAJOR;
}

int ofc_pi::GetPlugInVersionMinor()
{
      return PLUGIN_VERSION_MINOR;
}

wxBitmap *ofc_pi::GetPlugInBitmap()
{
      return m_pplugin_icon;
}

wxString ofc_pi::GetCommonName()
{
    return _("Fugawi Charts");
}
...
wxArrayString ofc_pi::GetDynamicChartClassNameArray()
{
      return m_class_name_array;
}


void ofc_pi::OnSetupOptions( void )
{
...
bool validate_server(void)
{
    ...
    wxLogMessage(_T("ofc_pi: starting serverd utility: ") + cmds);
    g_serverProc = wxExecute(cmds, flags);              // exec asynchronously
    wxMilliSleep(1000);
    
    
#else           // Android
    qDebug() << "Starting ofc_pi server";
